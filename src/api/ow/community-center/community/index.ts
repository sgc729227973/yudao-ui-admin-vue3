...
export const getCommunityList = async (params) => {
  return await request.get({ url: '/ow/community/release/page', params }, true);
};

// 获取社区详情
export const getCommunityDetail = async (id: number) => {
  return await request.get({ url: '/ow/community/release/get', params: { id } }, true);
};

export const createCommunity = async (data: CommunityVO) => {
  return await request.post({ url: '/ow/community/release/create', data }, true);
};

export const updateCommunity = async (id: number, data: CommunityVO) => {
  return await request.put({ url: '/ow/community/release/update', data: { ...data, id } }, true);
};

export const deleteCommunity = async (id: number) => {
  return await request.delete({ url: '/ow/community/release/delete', params: { id } }, true);
};

export const exportCommunity = (params) => {
  return request.download({ url: '/ow/community/release/export', params }, true);
};
