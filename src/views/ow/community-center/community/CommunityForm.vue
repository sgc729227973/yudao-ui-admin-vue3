        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="载体类型" prop="mediaType">
              <el-select v-model="formData.mediaType" placeholder="请选择载体类型">
                <el-option
                  v-for="carrier in getIntDictOptions(DICT_TYPE.OW_COMMUNITY_TYPE)"
                  :key="carrier.value"
                  :label="carrier.label"
                  :value="carrier.value"
                />
...
import { getSimpleTagList } from '@/api/ow/community-center/community-tag';
import {
  createCommunity,
  updateCommunity,
  getCommunityDetail,
  CommunityVO,
  OfficialWebsiteTag,
} from '@/api/ow/community-center/community';
import { ref, reactive, defineEmits, defineExpose, onMounted, watch } from 'vue';
import CommentsSectionForm from './CommentsSectionForm.vue'; // 评论管理组件
...
const formRules = reactive({
  ...
  mediaType: [{ required: true, message: '载体类型不能为空', trigger: 'change' }],
});
