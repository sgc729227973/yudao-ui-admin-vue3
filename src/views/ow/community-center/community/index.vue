<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="人群类别" prop="category">
        <el-select
        ...
        >
          <el-option
            v-for="category in getIntDictOptions(DICT_TYPE.OW_GROUP_CATEGORY)"
            :key="category.value"
      ...
      <!-- 载体类型 -->
      <el-form-item label="载体类型" prop="mediaType">
        <el-select
        ...
        >
          <el-option
            v-for="carrier in getIntDictOptions(DICT_TYPE. OW_MEDIA_TYPE)"
            :key="carrier.value"
            :label="carrier.label"
            :value="carrier.value"
          />
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="filteredData" stripe>
      ...
      <el-table-column align="center" label="知识类型" prop="type">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.OW_SERVICES_NAME" :value="scope.row.type" />
        </template>
      </el-table-column>
      <el-table-column align="center" label="人群类别" prop="category">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.OW_GROUP_CATEGORY" :value="scope.row.category" />
        </template>
      </el-table-column>
      <el-table-column align="center" label="载体类型" prop="mediaType">
        <template #default="scope">
...
import * as CommunityApi from '@/api/ow/community-center/community'
